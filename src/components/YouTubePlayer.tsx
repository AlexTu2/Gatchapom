...
export function YouTubePlayer() {
  const { state: timerState, phase: currentPhase } = useTimer();
  ...
  const syncWithTimer = useCallback(() => {
    ...
    const shouldPlay = playModes[currentPhase as keyof PlayModes];
    
    try {
      if (timerState === 'paused' || !shouldPlay) {
        const playerState = iframeApi.getPlayerState();
        // Only pause if currently playing (1 = playing)
        if (playerState === 1) {
          iframeApi.pauseVideo();
        }
      } else if (timerState === 'running' && shouldPlay) {
        const playerState = iframeApi.getPlayerState();
  ...
  }, [timerState, currentPhase, isSync, playModes, iframeApi, isPlayerReady]);
      ...
      <CardContent className="space-y-4">
        <div className="rounded-lg overflow-hidden aspect-video">
          <LiteYouTubeEmbed
            ...
            noCookie={false}
            params="autoplay=1&enablejsapi=1&controls=1&origin=http://localhost:5173"
